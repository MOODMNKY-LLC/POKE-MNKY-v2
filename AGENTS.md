...
- To align local Postgres schema with linked prod when local migration history is ahead: park migrations not on remote under `supabase/migrations/_held_local_only_not_on_prod/`, then `supabase db pull` and `supabase db reset`; for prod **row data** locally run `pnpm db:sync:prod-data` (`scripts/sync-prod-data-to-local.ps1`: linked dump → `db reset --no-seed` → restore). If Supabase MCP `apply_migration` used a different version timestamp than the repo file, rename the local migration to match `supabase_migrations.schema_migrations` before `db push` (avoid `migration repair --reverted` on already-applied remote migrations).
