...
- League **teams** via Admin → **Google Sheets** (`POST /api/sync/google-sheets`): enable **Data** tab only (**Select recommended** on `/admin/google-sheets`); policy in `lib/google-sheets-sheet-policy.ts` (do not sync Team 1–12 to `teams`). **Data** tab columns are index-based in `lib/google-sheets-data-tab.ts` (duplicate header labels break SOS/division if mapped by name). Partial success is normal when optional enabled sheets error but **Data** still syncs teams.
...
- In-app draft ops: **Cold start** (empty `draft_pool` + `pokemon_master`) — Admin **Seed from Showdown** / `POST /api/admin/draft-pool/bootstrap` (`lib/draft-pool-bootstrap.ts`): `lib/pokemon-catalog-hydrate` invokes Edge Functions `ingest-showdown-pokedex` and `sync-pokemon-pokeapi` via `lib/supabase/invoke-edge-function.ts` (local `sb_secret_*` keys need `apikey` + `Authorization`; `verify_jwt = false` in `supabase/config.toml`), pokenode-ts fallback → RPC `populate_draft_pool_from_showdown_tiers` (wraps `populate_showdown_pool_from_tiers`; migration `20260519120000_restore_*`) → `lib/draft-pool-trim` by generation (dex-range fallback if `draft_pool.generation` missing). Then **Generate** (auto `POST /api/admin/pokemon-master/backfill` when master empty), **Publish** (`season_draft_pool` → `draft_pool`). Leave **Game code** empty unless `pokemon_games` is populated. Guides: `docs/DRAFT-IN-APP-OPERATIONS.md`, `docs/GOOGLE-SHEETS-SYNC-GUIDE.md`, `docs/SESSION-CHANGELOG-2026-05-19.md`.
