...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.error("[Pokepedia Sprite Worker] Auth error:", authError);
    ...
    try {
      ...
      if (!userIsAdmin) {
        ...
        if (profileError) {
          console.error("[Pokepedia Sprite Worker] Profile lookup error:", profileError);
        } else {
          userIsAdmin = profile?.role === "admin";
          console.log("[Pokepedia Sprite Worker] Admin check (service role):", {
            userId: user.id,
      ...
      console.error("[Pokepedia Sprite Worker] Error checking admin status:", adminCheckError);
      ...
      try {
        ...
        console.error("[Pokepedia Sprite Worker] Fallback admin check failed:", fallbackError);
        return NextResponse.json(
