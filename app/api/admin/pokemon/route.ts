 ...
 * Save Pokémon availability and tier changes to draft_pool
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[Admin Pokemon API] Upsert error:", error)
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      message: `Successfully updated ${data?.length || 0} Pokémon`,
    })
  } catch (error: any) {
    console.error("[Admin Pokemon API] Error:", error)
    return NextResponse.json(
...
/**
 * DELETE /api/admin/pokemon
 * Reset/clear all draft_pool entries for a season
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete all draft_pool entries for the season
    ...
    if (deleteError) {
      console.error("[Admin Pokemon API] Delete error:", deleteError)
      return NextResponse.json(
        { error: `Failed to reset draft pool: ${deleteError.message}` },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully reset draft pool for season`,
      deleted_count: count || 0,
    })
  } catch (error: any) {
    console.error("[Admin Pokemon API] Error:", error)
    return NextResponse.json(
      { error: error.message || "Failed to reset draft pool" },
      { status: 500 }
    )
  }
}
