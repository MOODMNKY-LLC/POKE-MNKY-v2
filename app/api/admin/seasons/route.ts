 ...
 * POST /api/admin/seasons — create season + optional team slot generation
 */
...
export async function GET() {
  try {
    ...
    const { data, error } = await service
      .from("seasons")
      .select(
        "id, name, season_id, start_date, end_date, is_current, conference_count, division_count, team_slot_count, created_at"
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    const divisionCount = typeof body?.division_count === "number" ? body.division_count : 4
    const teamSlotCount =
    ...
    if (conferenceCount < 1 || divisionCount < 1 || teamSlotCount < 1) {
      return NextResponse.json(
        { error: "conference_count, division_count, and team_slot_count must be >= 1" },
        { status: 400 }
      )
    }
    ...
    const { season, teamGeneration } = await createSeasonWithStructure(service, {
      name,
      startDate,
      endDate,
      setAsCurrent,
