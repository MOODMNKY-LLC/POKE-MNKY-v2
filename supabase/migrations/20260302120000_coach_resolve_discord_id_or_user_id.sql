-- Coach resolution: allow bot RPC to resolve coach by discord_user_id OR discord_id
-- so /pick works when admin linked Discord via profile + assign_coach (which sets coaches.discord_id)
...
COMMENT ON FUNCTION public.rpc_discord_submit_draft_pick IS 'Bot-only RPC for Discord draft picks; resolves coach by discord_user_id or discord_id.';
