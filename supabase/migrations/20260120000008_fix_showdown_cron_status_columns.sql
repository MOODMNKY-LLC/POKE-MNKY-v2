-- Fix get_showdown_pokedex_cron_status() - cron.job table doesn't have last_run/next_run columns
...
COMMENT ON FUNCTION public.get_showdown_pokedex_cron_status() IS 'Get status of Showdown pokedex cron job - requires SECURITY DEFINER to access cron schema. last_run comes from job_run_details, next_run is NULL (not available in pg_cron)';
