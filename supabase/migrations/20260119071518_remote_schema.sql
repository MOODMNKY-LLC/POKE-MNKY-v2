do $$ begin if not exists (select 1 from pg_type where typname = 'color_source' and typnamespace = (select oid from pg_namespace where nspname = 'public')) then create type "public"."color_source" as enum ('99COLORS_NET', 'ART_PAINTS_YG07S', 'BYRNE', 'CRAYOLA', 'CMYK_COLOR_MODEL', 'COLORCODE_IS', 'COLORHEXA', 'COLORXS', 'CORNELL_UNIVERSITY', 'COLUMBIA_UNIVERSITY', 'DUKE_UNIVERSITY', 'ENCYCOLORPEDIA_COM', 'ETON_COLLEGE', 'FANTETTI_AND_PETRACCHI', 'FINDTHEDATA_COM', 'FERRARIO_1919', 'FEDERAL_STANDARD_595', 'FLAG_OF_INDIA', 'FLAG_OF_SOUTH_AFRICA', 'GLAZEBROOK_AND_BALDRY', 'GOOGLE', 'HEXCOLOR_CO', 'ISCC_NBS', 'KELLY_MOORE', 'MATTEL', 'MAERZ_AND_PAUL', 'MILK_PAINT', 'MUNSELL_COLOR_WHEEL', 'NATURAL_COLOR_SYSTEM', 'PANTONE', 'PLOCHERE', 'POURPRE_COM', 'RAL', 'RESENE', 'RGB_COLOR_MODEL', 'THOM_POOLE', 'UNIVERSITY_OF_ALABAMA', 'UNIVERSITY_OF_CALIFORNIA_DAVIS', 'UNIVERSITY_OF_CAMBRIDGE', 'UNIVERSITY_OF_NORTH_CAROLINA', 'UNIVERSITY_OF_TEXAS_AT_AUSTIN', 'X11_WEB', 'XONA_COM'); end if; end $$;

do $$ begin if not exists (select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relname = 'documents_id_seq' and c.relkind = 'S') then create sequence "public"."documents_id_seq"; end if; end $$;

do $$ begin if not exists (select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relname = 'nods_page_id_seq' and c.relkind = 'S') then create sequence "public"."nods_page_id_seq"; end if; end $$;

do $$ begin if not exists (select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relname = 'nods_page_section_id_seq' and c.relkind = 'S') then create sequence "public"."nods_page_section_id_seq"; end if; end $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS colors_pkey ON public.colors USING btree (id);

CREATE UNIQUE INDEX IF NOT EXISTS documents_pkey ON public.documents USING btree (id);

CREATE UNIQUE INDEX IF NOT EXISTS nods_page_path_key ON public.nods_page USING btree (path);

CREATE UNIQUE INDEX IF NOT EXISTS nods_page_pkey ON public.nods_page USING btree (id);

CREATE UNIQUE INDEX IF NOT EXISTS nods_page_section_pkey ON public.nods_page_section USING btree (id);

CREATE UNIQUE INDEX IF NOT EXISTS todos_pkey ON public.todos USING btree (id);

do $$ begin if not exists (select 1 from pg_constraint where conname = 'colors_pkey') then alter table "public"."colors" add constraint "colors_pkey" PRIMARY KEY using index "colors_pkey"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'documents_pkey') then alter table "public"."documents" add constraint "documents_pkey" PRIMARY KEY using index "documents_pkey"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'nods_page_pkey') then alter table "public"."nods_page" add constraint "nods_page_pkey" PRIMARY KEY using index "nods_page_pkey"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'nods_page_section_pkey') then alter table "public"."nods_page_section" add constraint "nods_page_section_pkey" PRIMARY KEY using index "nods_page_section_pkey"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'todos_pkey') then alter table "public"."todos" add constraint "todos_pkey" PRIMARY KEY using index "todos_pkey"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'nods_page_parent_page_id_fkey') then alter table "public"."nods_page" add constraint "nods_page_parent_page_id_fkey" FOREIGN KEY (parent_page_id) REFERENCES public.nods_page(id) not valid; end if; end $$;

alter table "public"."nods_page" validate constraint "nods_page_parent_page_id_fkey";

do $$ begin if not exists (select 1 from pg_constraint where conname = 'nods_page_path_key') then alter table "public"."nods_page" add constraint "nods_page_path_key" UNIQUE using index "nods_page_path_key"; end if; end $$;

do $$ begin if not exists (select 1 from pg_constraint where conname = 'nods_page_section_page_id_fkey') then alter table "public"."nods_page_section" add constraint "nods_page_section_page_id_fkey" FOREIGN KEY (page_id) REFERENCES public.nods_page(id) ON DELETE CASCADE not valid; end if; end $$;

alter table "public"."nods_page_section" validate constraint "nods_page_section_page_id_fkey";

do $$ begin if not exists (select 1 from pg_constraint where conname = 'todos_task_check') then alter table "public"."todos" add constraint "todos_task_check" CHECK ((char_length(task) > 3)) not valid; end if; end $$;

alter table "public"."todos" validate constraint "todos_task_check";

do $$ begin if not exists (select 1 from pg_constraint where conname = 'todos_user_id_fkey') then alter table "public"."todos" add constraint "todos_user_id_fkey" FOREIGN KEY (user_id) REFERENCES auth.users(id) not valid; end if; end $$;
